...
sd_pwr_ctrl_ldo_config_t ldo_config = {.ldo_chan_id=0};

// --------------------------------------------
// 
/*
  New: release on chip LDO.
  Re-mounting cannot take place until released.
*/
// --------------------------------------------
void deleteLDOPowerHandle() {
  if (sd_pwr_ctrl_del_on_chip_ldo(host.pwr_ctrl_handle) != ESP_OK) {
    log_e("Failed to delete on-chip LDO power control driver");
  }
  else {
    // log_e("Successfully deleted on-chip LDO power control driver");
...
// New: expose card
// --------------------------------------------
sdmmc_card_t *fs::SDMMCFS::returnCard() {
  return _card;
}
// --------------------------------------------
// New: expose host
// --------------------------------------------
sdmmc_host_t SDMMCFS::returnHost() {
  return host;
}
// --------------------------------------------
// New: expose slot config
// --------------------------------------------
sdmmc_slot_config_t SDMMCFS::returnSlotConfig() {
  return slot_config;
}
// --------------------------------------------
// New: expose LDO config
// --------------------------------------------
sd_pwr_ctrl_ldo_config_t SDMMCFS::returnLDOConfig() {
  return ldo_config;
}
// --------------------------------------------
...
bool SDMMCFS::begin(const char *mountpoint, bool mode1bit, bool format_if_mount_failed,
  ...
  if (_power_channel == -1) {
    ...
    // --------------------------------------------
    // New: store pwr_ctrl_handle for end()
    // --------------------------------------------
    ldo_config = {
      .ldo_chan_id = _power_channel,
    };
    sd_pwr_ctrl_handle_t tmp_pwr_ctrl_handle = NULL;
    if (sd_pwr_ctrl_new_on_chip_ldo(&ldo_config, &tmp_pwr_ctrl_handle) != ESP_OK) {
      log_e("Failed to create a new on-chip LDO power control driver");
      deleteLDOPowerHandle();
      return false;
    }
    else {
      // log_e("Successfully created on-chip LDO power control driver");
      host.pwr_ctrl_handle = tmp_pwr_ctrl_handle;
    }
    // --------------------------------------------
  }
#endif
...
#error "BOARD_SDMMC_POWER_ON_LEVEL not defined, please define it in pins_arduino.h"
  ...
  esp_vfs_fat_sdmmc_mount_config_t mount_config = {
    ...
    .disk_status_check_enable = disk_status_check_enable,
    .use_one_fat = false
  };
...
#endif
    // required true in cases where new LDO cannel cannot be aquired.
    deleteLDOPowerHandle();
  }
}
