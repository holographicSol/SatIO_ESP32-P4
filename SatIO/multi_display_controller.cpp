...
void updateIndicator(TwoWire &wire, int address, int display_index, int r, int g, int b) {
    ...
    writeI2CToSlave(wire, IICLinkMultiDisplayController, address, 4, "updateIndicator");
}
...
void update7Segment4Digit(TwoWire &wire, int address, int display_index, char* value) {
    ...
    writeI2CToSlave(wire, IICLinkMultiDisplayController, address, 4, "update7Segment4Digit");
}
...
void update7Segment6Digit(TwoWire &wire, int address, int display_index, char* value) {
    ...
    writeI2CToSlave(wire, IICLinkMultiDisplayController, address, 4, "update7Segment6Digit");
}
...
void updateSSD1306(TwoWire &wire, int address, int display_index, int value_index, int dx, int dy, char* value) {
    ...
    writeI2CToSlave(wire, IICLinkMultiDisplayController, address, 4, "updateSSD1306");
}
...
void drawSSD1306Canvas(TwoWire &wire, int address, int display_index) {
  ...
  strcat(IICLinkMultiDisplayController.OUTPUT_BUFFER_CHARS, String(display_index).c_str());
  writeI2CToSlave(wire, IICLinkMultiDisplayController, address, 4, "drawSSD1306Canvas");
}
...
void requestMultiDisplayControllerData(TwoWire &wire, int address, bool &isr_interrupt_flag) {
 if (isr_interrupt_flag==true) {
   ...
   requestFromSlave(wire, IICLinkMultiDisplayController, address, 301, 5, 0, "requestMultiDisplayControllerData");
   for (int i = 0; i<IICLinkMultiDisplayController.i_token; i++) {
     ...
     else if (i==1 && str_is_bool(IICLinkMultiDisplayController.TOKENS[i])) {ALLOW_UPDATE_MULTIDISPLAY_CONTROLLER_0=atoi(IICLinkMultiDisplayController.TOKENS[i]);}
    }
